...
export const subscribe = <T>(path: string, callback: (items: T[]) => void, constraints: Parameters<typeof query>[1][] = []) => onSnapshot(query(collection(db, path), ...constraints), s => callback(rows<T>(s)));
export const subscribeStudentSnapshot = (uid: string, callback: (data: DocumentData | undefined) => void) => onSnapshot(doc(db, "students", uid), s => callback(s.data()));
...
export const subscribeUsers = (callback: (items: DocumentData[]) => void) => onSnapshot(collection(db, "users"), s => callback(rows<DocumentData>(s)));
export const subscribeActiveSession = (uid: string, callback: (data: DocumentData | undefined) => void) => onSnapshot(doc(db, "active_sessions", uid), s => callback(s.data()));
